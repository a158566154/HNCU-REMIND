...
def select_money():
    ...
    if money[0] < 10 :
        ...
        SendMessage(Subject='# 电费不足10元提醒',Content='### 点击<a href=\{}>这里</a>进行电费缴纳'.format(power_url)
        print(power_money)
        time.sleep(3600)
        ...
        print(woter_money)
        SendMessage(Subject='# 水费不足10元提醒',Content='### 点击<a href=\{}>这里</a>进行水费缴纳'.format(woter_url)

        time.sleep(3600)
    else:
        #SendMessage(Subject='# 水费不足10元提醒',Content='### 点击<a href=\"http://smart.hncu.net/microhomes/classic/html/index.html#?publicUserId=admin&weixin_openid=oKAGAwzqqC9AGIMd-rTYDJHaJuEY&tab=5\">这里</a>进行水费缴纳')
